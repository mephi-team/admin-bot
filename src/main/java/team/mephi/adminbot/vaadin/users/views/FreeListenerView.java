...
/**
 * Представление для отображения слушателей в системе.
 * Расширяет абстрактный класс AbstractGridView для работы с сущностями SimpleUser.
 */
public class FreeListenerView extends AbstractGridView<SimpleUser> {
    ...
    public FreeListenerView(StudentPresenter actions) {
        ...
        var config = GridViewConfig.<SimpleUser>builder()
                ...
                .searchPlaceholder(getTranslation("grid_free_listener_search_placeholder"))
                .emptyLabel(getTranslation("grid_free_listener_empty_label"))
    ...
    protected void configureActionColumn(com.vaadin.flow.component.grid.Grid<SimpleUser> grid) {
        grid.addComponentColumn(item -> {
            Button dropButton = new TextButton(getTranslation("grid_student_action_drop_label"), VaadinIcon.CLOSE.create(), ignoredEvent -> actions.onExpel(List.of(item.getId()), DialogType.EXPEL_USERS));
