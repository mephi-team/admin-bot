...
public class CandidateView extends VerticalLayout {
    ...
    public CandidateView(UsersPresenter actions) {
        ...
        grid.addColumn(MyRenderers.createPdRenderer()).setHeader(getTranslation("grid_candidate_header_pd_consent_label")).setSortable(true).setResizable(true).setKey("pdConsent");
        ...
        grid.addColumn(u -> Objects.nonNull(u.getDirection()) ? u.getDirection().stream().map(SimpleDirection::getName).collect(Collectors.joining(",")) : "").setHeader(getTranslation("grid_candidate_header_direction_label")).setSortable(true).setResizable(true).setKey("direction");
        ...
        grid.addComponentColumn(item -> {
            Button confirmButton = new Button(VaadinIcon.CHECK.create(), e -> actions.onAccept(List.of(item.getId()), DialogType.ACCEPT_USERS));
            Button rejectButton = new Button(VaadinIcon.CLOSE.create(), e -> actions.onReject(List.of(item.getId()), DialogType.REJECT_USERS));
