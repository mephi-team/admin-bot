...
public class LeftMenu extends SideNav {
    ...
    public LeftMenu(AuthService authService) {
        ...
        if (authService.isAdmin()) {
            var item = new SideNavItem(getTranslation("left_menu_dialogs_link"), Dialogs.class, VaadinIcon.COMMENT_O.create());
            item.setMatchNested(true);
            addItem(item, new SideNavItem(getTranslation("left_menu_mailing_link"), Mailings.class, VaadinIcon.ENVELOPE_O.create()));
        }
        addItem(new SideNavItem(getTranslation("left_menu_question_link"), Questions.class, VaadinIcon.COMMENTS_O.create()));
    }
}
