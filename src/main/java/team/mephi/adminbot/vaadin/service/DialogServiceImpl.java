...
public class DialogServiceImpl<T> implements DialogService<T> {
    ...
    @Override
    public void showConfirmDialog(Object item, String label, SerializableConsumer<T> callback) {
        dialogFactory.getConfirmDialog(label).showForConfirm(item, () -> callback.accept((T) item));
    }
}
