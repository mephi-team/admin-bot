...
public class DataInitializer {
    ...
    private void initUsers() {
        ...
        List<User> users = Arrays.asList(
                User.builder().tgId("tg_1001").tgName("tg_name_1001").email("test1@example.com").userName("Анна Смирнова").firstName("Анна").lastName("Смирнова").role(studentRole).direction(java).status(UserStatus.ACTIVE).build(),
                User.builder().tgId("tg_1002").tgName("tg_name_1002").email("test2@example.com").userName("Иван Петров").firstName("Иван").lastName("Петров").role(candidateRole).direction(analytics).status(UserStatus.ACTIVE).build(),
                User.builder().tgId("tg_1003").tgName("tg_name_1003").email("test3@example.com").userName("Мария Козлова").firstName("Мария").lastName("Козлова").role(candidateRole).direction(python).status(UserStatus.BLOCKED).build(),
                User.builder().tgId("tg_1004").tgName("tg_name_1004").email("test4@example.com").userName("Алексей Иванов").firstName("Алексей").lastName("Иванов").role(middleCandidateRole).direction(java).status(UserStatus.ACTIVE).build(),
                User.builder().tgId("tg_1005").tgName("tg_name_1005").email("test5@example.com").userName("Екатерина Волкова").firstName("Екатерина").lastName("Волкова").role(studentRole).direction(analytics).status(UserStatus.ACTIVE).build(),
